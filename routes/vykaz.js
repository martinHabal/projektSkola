...
router.get("/vykaz-novy-novy", async (req, res) => {
  ...
  try {
    ...
    const [odevzdano] = await pool.query(`SELECT EXISTS(SELECT 1 FROM odevzdano 
        ...
        ) AS maOdevzdano`,[req.session.user.id]
    );
...
router.post("/api/uloz-vykaz", async (req, res) => {
  ...
  const dataMaped = data
    ...
    .map((zaznam) => {
      const [den, mesic, rok] = zaznam.Datum.split(".")
        .map((s) => s.trim())
        .filter(Boolean);
...
router.get("/work-report", async (req, res) => {
  try {
    ...
    const employeeName =
      employee.length > 0 ? employee[0].username : "Neznámý zaměstnanec";

    // 2. Získání pracovních záznamů
    const [records] = await pool.query(
      //tady dodelat aby se vytisknul aktualni mesic a rok, ne jenom vsechny zaznamy
      `SELECT *
     FROM work_logs
     WHERE users_id = ?
